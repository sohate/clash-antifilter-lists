<?php
	...
	function convert_clash() {
		$fromFile = '1st_of_all.list';
		$toFile = '1st_of_all.yaml';
...
<html>
	<head>
		...
		<title>Clash & Shadowrocket rules</title>
				...
				postAjax("?",{"save":"1","fileName":fileName,"content":document.getElementById('editor').value},function(data) {
					...
					try { data = JSON.parse(data); }
						...
						window.setTimeout(function() {
							$('#saveStatus').fadeOut(
								function() {
									document.getElementById('saveStatus').innerHTML = 'Save rules';
	...
	<body>
		<div class="topnav">
			<div class="left-container">
				<button id = 'saveStatus' onclick = 'doSave();'>Save rules</button>
			</div>
